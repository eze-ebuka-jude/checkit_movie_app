...
async function getAll(type: string, id: string) {
...
export default async function DetailsPage({
    ...
    const data = await getAll(type, id);
    ...
    return (
        <div className="max-w-7xl! mx-auto! px-4! py-8! text-white">
            ...
            <div className="relative">
                ...
                <div className="bg-gray-100/20! border rounded-lg absolute left-8 backdrop-blur-sm! -bottom-16 w-[40%] h-auto p-5! text-white! font-bold text-2xl">
                    <span className="text-sm!">CheckitMovies / {type}</span>
                    <h1 className="py-3! font-bold text-md!">{type === 'movies' ? data.title : data.name}</h1>
                </div>
            </div>
                    ...
                    {type === "movies" ? (
                        <>
                            <div className="flex flex-col gap-4!">
                                ...
                                <h3 className="text-2xl! font-bold">{type === "movies" ? "Movies" : "TV Shows"}</h3>
                            </div>
